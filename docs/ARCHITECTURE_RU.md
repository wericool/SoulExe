...
flowchart TD
  ...
  Notify[Android foreground service] --> API
...
Мобильное приложение хранит только собственные локальные параметры подключения и оформления. История чатов, карточки и аватары берутся с ПК через LAN API. Нативный Android foreground service периодически проверяет этот же API и показывает уведомления о новых сообщениях персонажей, когда приложение скрыто.

## Как работает синхронизация

Синхронизация не использует облако и не требует WebSocket-сервера. Mobile-клиент регулярно получает актуальные списки и активную ленту, а Windows-клиент после операции с телефона получает локальное уведомление и обновляет свои данные. Foreground service работает по тому же принципу и не является облачным push: если ПК выключен или недоступен по LAN/VPN, новые уведомления прийти не смогут. Такой подход сохраняет один источник истины — Windows SoulExe.
