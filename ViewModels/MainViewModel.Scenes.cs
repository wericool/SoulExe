...
public sealed partial class MainViewModel
{
    ...
    private async Task LoadSelectedSceneAsync(Guid? sceneId)
    {
        ...
        await UpdateSceneUiAsync(() =>
        {
            ...
            _sceneMessageWindowStart = ConversationPresentationWindow.LatestStart(snapshot.Messages.Count);
            RebuildSceneMessageWindow();
    ...
    private Task RequestSelectedSceneDeletionAsync()
    {
        var editor = SelectedGroupConversation;
        if (editor is null) return Task.CompletedTask;
        PendingDeletion = new PendingDeletionRequest("Удалить групповой разговор?", $"Групповой разговор «{editor.Name}» будет удалён.", "Это действие нельзя отменить: история, параметры и локальные данные разговора будут удалены.", () => DeleteSceneAsync(editor.Id));
        return Task.CompletedTask;
    }
    private async Task DeleteSceneAsync(Guid sceneId)
    {
        var editor = SelectedGroupConversation;
        if (editor?.Id != sceneId) return;
        try
        {
            CancelSceneTimer();
    ...
    private void SelectSceneMessageSearchResult(ChatMessageSearchResult? result)
    {
        ...
        EnsureSceneMessageVisible(result.MessageId);
    ...
    private void LoadOlderSceneMessages()
    {
        if (SelectedGroupConversation is null || !HasOlderSceneMessages) return;
        _sceneMessageWindowStart = ConversationPresentationWindow.PreviousStart(_sceneMessageWindowStart);
        RebuildSceneMessageWindow();
    }

    private void EnsureSceneMessageVisible(Guid messageId)
    {
        var conversation = SelectedGroupConversation?.Conversation;
        ...
        var start = ConversationPresentationWindow.StartContaining(conversation.Messages.Count, _sceneMessageWindowStart, index);
        if (start == _sceneMessageWindowStart) return;
        _sceneMessageWindowStart = start;
        RebuildSceneMessageWindow();
    }

    private void RebuildSceneMessageWindow()
    {
        SceneMessages.Clear();
        if (SelectedGroupConversation is not null)
            foreach (var view in SceneMessageTimeline.BuildRange(SelectedGroupConversation.Conversation, Characters, _sceneMessageWindowStart, 60))
                SceneMessages.Add(view);
        OnPropertyChanged(nameof(HasOlderSceneMessages));
        LoadOlderSceneMessagesCommand?.RaiseCanExecuteChanged();
    }

    private void AddScenePresentationMessage(SceneMessageViewModel message)
    {
        if (SceneMessages.Count == ConversationPresentationWindow.PageSize) SceneMessages.RemoveAt(0);
