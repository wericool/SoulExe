...
describe("SoulExe demo API", () => {
  ...
  it("выдаёт чаты и сцены через общий список Conversation", async () => {
    ...
    const direct = conversations.find((conversation) => conversation.mode === "personal");
    const scene = conversations.find((conversation) => conversation.mode === "group");
  ...
  it("отправляет реплику через единый API разговора", async () => {
    const api = createSoulExeDemoApi();
    const conversation = (await api.getConversations()).find((item) => item.mode === "personal");
    if (!conversation) throw new Error("В демо нет обычного разговора.");

    const updated = await api.sendConversationMessage(conversation.id, "Продолжим?");

    expect(updated.messages.at(-1)).toMatchObject({ author: "Луна", kind: "message" });
  });
  ...
  it("выполняет сценические действия через единый API разговора", async () => {
    const api = createSoulExeDemoApi();
    const scene = (await api.getConversations()).find((item) => item.mode === "group");
    if (!scene) throw new Error("В демо нет сцены.");

    const updated = await api.conversationAction(scene.id, { action: "director", text: "Гаснет свет." });

    expect(updated.messages.at(-1)).toMatchObject({ kind: "director", content: "Гаснет свет." });
  });
