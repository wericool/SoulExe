...
export type SoulCharacterDraft = Pick<
  SoulCharacter,
  | "name"
  | "title"
  | "description"
  ...
  | "soulMemoryEnabled"
  | "soulMemoryPreset"
  | "soulMemoryIntervalMessages"
  | "autoSummaryEnabled"
  | "autoSummaryIntervalMessages"
...
export type SoulLorebookSummary = {
  id: string;
  name: string;
  description?: string;
  entriesCount: number;
...
export type SoulPersonaDraft = Pick<
  SoulPersona,
...
export type SoulConversationGenerationPreview = {
  text: string;
...
export type UpdateSoulConversationRequest =
  Partial<CreateSoulConversationRequest>;
...
export function cleanSceneContent(value: string): string {
  return (value || "")
    ...
    .replace(
      /^\s*\[(?:SCENE STATE|RELATIONSHIP DYNAMICS|ACTIVE CHARACTER LORE|CURRENT SPEAKER|COUNTERPART|SCENE CONTRACT|SCENE PROGRESSION|MANDATORY PROGRESSION CHECK)[^\]]*\]\s*$/gim,
      "",
    )
...
export async function checkSoulExeServer(
  baseUrl: string,
  timeoutMs = 1000,
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (health.service !== "SoulExe")
        throw new Error("По этому адресу не найден SoulExe.");
      return health;
    }
...
export class SoulExeApiClient {
  ...
  private withPersonaAvatarUrl(
    persona?: SoulPersona | null,
  ): SoulPersona | null | undefined {
    if (!persona || !persona.avatarUrl) return persona;
    return {
      ...persona,
      avatarUrl: this.withAvatarSessionUrl(persona.avatarUrl),
    };
  }

  private withConversationAvatarUrls(
    ...
    return {
      ...
      participants: conversation.participants.map((participant) => ({
        ...participant,
        avatarUrl: this.withAvatarSessionUrl(participant.avatarUrl),
      })),
    };
  }
  ...
  private async request<T>(
    ...
    const response = await fetch(
      `${normalizeServerUrl(this.session.baseUrl)}${path}`,
      {
        ...options,
        cache: "no-store",
        headers: {
  ...
  async getCharacters(signal?: AbortSignal) {
    ...
    return characters.map(
      (character) => this.withAvatarUrl(character) as SoulCharacter,
    );
  }

  async getPersonas() {
    const personas = await this.request<SoulPersona[]>("/api/personas");
    return personas.map(
      (persona) => this.withPersonaAvatarUrl(persona) as SoulPersona,
    );
  }

  async getPromptPresets() {
    return this.request<SoulPromptPreset[]>("/api/prompt-presets");
  }

  async getLorebooks() {
    return this.request<SoulLorebookSummary[]>("/api/lorebooks");
  }

  registerPushToken(token: string, deviceName?: string) {
    return this.request<{ registered: boolean }>("/api/push/register", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ token, platform: "android", deviceName }),
    });
  }

  unregisterPushToken(token: string) {
    return this.request<{ registered: boolean }>("/api/push/unregister", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  async createPersona(
    draft: Pick<SoulPersonaDraft, "name"> & Partial<SoulPersonaDraft>,
  ) {
    return this.withPersonaAvatarUrl(
      await this.request<SoulPersona>("/api/personas", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  async generatePersona(idea: string) {
    return this.withPersonaAvatarUrl(
      await this.request<SoulPersona>("/api/personas/generate", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  async expandPersonaDescription(idea: string) {
    return this.request<{ description: string }>("/api/personas/expand", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ idea }),
    });
  ...
  async updatePersona(personaId: string, draft: Partial<SoulPersonaDraft>) {
    return this.withPersonaAvatarUrl(
      await this.request<SoulPersona>(`/api/personas/${personaId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(draft),
      }),
  ...
  async getConversations(take?: number) {
    const suffix =
      take && take > 0
        ...
        : "";
    const conversations = await this.request<SoulConversation[]>(
      `/api/conversations${suffix}`,
    );
    return conversations.map((conversation) =>
      this.withConversationAvatarUrls(conversation),
    );
  }
  ...
  async getConversation(conversationId: string, take?: number) {
    const suffix =
      take && take > 0
        ? `?take=${Math.min(Math.max(Math.trunc(take), 1), 100)}`
        : "";
    return this.withConversationAvatarUrls(
      await this.request<SoulConversation>(
  ...
  async getConversationPage(options: SoulConversationPageOptions = {}) {
    ...
    if (options.limit && options.limit > 0)
      params.set(
        "limit",
        String(Math.min(Math.max(Math.trunc(options.limit), 1), 100)),
      );
    if (options.take && options.take > 0)
      params.set(
        "take",
        String(Math.min(Math.max(Math.trunc(options.take), 1), 100)),
      );
    const suffix = params.size ? `?${params.toString()}` : "";
    const page = await this.request<SoulConversationPage>(
      `/api/conversations/page${suffix}`,
    );
    return {
      ...page,
  ...
  async startConversationAction(
    conversationId: string,
    action: SoulConversationAction,
  ) {
    await this.request<{ accepted: boolean }>(
      `/api/conversations/${conversationId}/actions?async=1`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(action),
  ...
  getConversationGenerationPreview(conversationId: string) {
    return this.request<SoulConversationGenerationPreview>(
      `/api/conversations/${conversationId}/generation-preview`,
    );
  }
  ...
  async createConversation(request: CreateSoulConversationRequest) {
    return this.withConversationAvatarUrls(
      await this.request<SoulConversation>("/api/conversations", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  async updateConversation(
    conversationId: string,
    request: UpdateSoulConversationRequest,
  ) {
    return this.withConversationAvatarUrls(
      await this.request<SoulConversation>(
  ...
  sendConversationMessage(
    conversationId: string,
    text: string,
    author: Pick<SoulConversationAction, "authorKind" | "authorPersonaId"> = {},
  ) {
    return this.conversationAction(conversationId, {
      action: "send",
      text,
      ...author,
    });
  ...
  async createCharacter(
    draft: Pick<SoulCharacterDraft, "name"> & Partial<SoulCharacterDraft>,
  ) {
    return this.withAvatarUrl(
      await this.request<SoulCharacter>("/api/characters", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(draft),
      }),
  ...
  async generateCharacter(idea: string) {
    return this.withAvatarUrl(
      await this.request<SoulCharacter>("/api/characters/generate", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ idea }),
      }),
  ...
  async updateCharacter(
    characterId: string,
    draft: Partial<SoulCharacterDraft>,
  ) {
    return this.withAvatarUrl(
      await this.request<SoulCharacter>(`/api/characters/${characterId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(draft),
      }),
  ...
  async expandCharacterField(
    characterId: string,
    field: "description" | "personality" | "scenario",
  ) {
    return this.withAvatarUrl(
      await this.request<SoulCharacter>(
        `/api/characters/${characterId}/expand`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  async uploadPersonaAvatar(personaId: string, asset: SoulAvatarUpload) {
    const extension =
      asset.fileName?.split(".").pop() ||
      (asset.mimeType === "image/png"
        ? "png"
    ...
    form.append("avatar", {
      uri: asset.uri,
      name: asset.fileName || `persona-avatar.${extension}`,
      type: asset.mimeType || "image/jpeg",
    } as unknown as Blob);
    return this.withPersonaAvatarUrl(
      await this.request<SoulPersona>(`/api/personas/${personaId}/avatar`, {
        method: "POST",
        body: form,
      }),
    ) as SoulPersona;
  ...
  async getScene(sceneId: string, signal?: AbortSignal) {
    const scene = await this.request<SoulScene>(
      `/api/scenes/${sceneId}?take=30`,
      { signal },
    );
  ...
  async createScene(request: CreateSoulSceneRequest) {
    return this.withSceneAvatarUrls(
      await this.request<SoulScene>("/api/scenes", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(request),
      }),
  ...
  async updateScene(sceneId: string, request: UpdateSoulSceneRequest) {
    return this.withSceneAvatarUrls(
      await this.request<SoulScene>(`/api/scenes/${sceneId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(request),
      }),
  ...
  private withSceneAvatarUrls(scene: SoulScene): SoulScene {
    return {
      ...
      messages: (scene.messages ?? []).map((message) => ({
        ...message,
        content: cleanSceneContent(message.content),
      })),
    };
  }
  ...
  async sceneAction(sceneId: string, action: "start" | "pause" | "next") {
    return this.withSceneAvatarUrls(
      await this.request<SoulScene>(`/api/scenes/${sceneId}/action`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action }),
      }),
  ...
  async addDirectorEvent(sceneId: string, text: string) {
    return this.withSceneAvatarUrls(
      await this.request<SoulScene>(`/api/scenes/${sceneId}/director`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
