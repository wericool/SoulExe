...
import { checkSoulExeServer, normalizeServerUrl, type ServerHealth } from "./soulexe-api";

export type DiscoveredSoulExeServer = {
  baseUrl: string;
  name: string;
  health: ServerHealth;
};
...
export async function discoverSoulExeServers(onStatus?: (message: string) => void): Promise<DiscoveredSoulExeServer[]> {
  ...
  const servers = await runBatches(hosts, 24, async (host) => {
    ...
    try {
      const health = await checkSoulExeServer(baseUrl, 650);
