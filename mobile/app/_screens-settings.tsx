...
function TypingSimulationSettings({ appearance, onChange }: { appearance: ChatAppearanceSettings; onChange: (changes: Partial<ChatAppearanceSettings>) => void }) {
  const options: Array<{ value: ChatAppearanceSettings["typingSimulation"]; label: string; description: string }> = [
    { value: "off", label: "Выключена", description: "Ответ показывается сразу после получения." },
    { value: "fast", label: "Быстро", description: "Ответ появляется короткими фрагментами." },
    { value: "slow", label: "Медленно", description: "Эффект печати заметнее для длинных ответов." },
  ];
  return <Card style={{ gap: 8, marginBottom: 12 }}><Text style={styles.settingTitle}>Видимость печати</Text><Text style={styles.helper}>Эта настройка влияет только на отображение уже полученного ответа и не замедляет сервер.</Text>{options.map((option) => <Pressable key={option.value} onPress={() => onChange({ typingSimulation: option.value })} style={({ pressed }) => [styles.markerToggle, pressed && styles.markerTogglePressed]}><View style={{ flex: 1 }}><Text style={styles.markerToggleTitle}>{option.label}</Text><Text style={styles.markerToggleSubtitle}>{option.description}</Text></View><View style={[styles.markerToggleTrack, appearance.typingSimulation === option.value && styles.markerToggleTrackOn]}><View style={[styles.markerToggleThumb, appearance.typingSimulation === option.value && styles.markerToggleThumbOn]} /></View></Pressable>)}</Card>;
}
