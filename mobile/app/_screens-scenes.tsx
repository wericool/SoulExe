...
export function ScenesScreen({ api, appearance, onThreadChange, initialSceneId, onBackToChats }: { api: SoulExeApi; appearance: ChatAppearanceSettings; onThreadChange: (open: boolean) => void; initialSceneId: string; onBackToChats: () => void }) {
  ...
  return <KeyboardAvoidingView style={styles.grow} behavior={Platform.OS === "ios" ? "padding" : undefined} keyboardVerticalOffset={0}>
    ...
    <ConversationMessageComposer value={directorText} onChangeText={setDirectorText} placeholder={messageAuthor.kind === "director" ? "Режиссёрское событие" : "Сообщение"} authorPicker={<ComposerAuthorPicker personas={personas} value={messageAuthor} onChange={setMessageAuthor} />} leftAction={{ icon: currentScene.status === "running" ? "pause" : "play-arrow", onPress: () => void action(currentScene.status === "running" ? "pause" : "start"), disabled: busy, accessibilityLabel: currentScene.status === "running" ? "Поставить сцену на паузу" : "Запустить сцену" }} rightActions={[{ icon: "send", primary: true, onPress: sendAuthoredMessage, disabled: busy || !directorText.trim(), accessibilityLabel: "Отправить сообщение" }, { icon: "arrow-upward", primary: true, onPress: () => void action("next"), disabled: busy, accessibilityLabel: "Следующая реплика" }]} />
    </View>
