...
public sealed class SoulDataRoot
{
    ...
    /// <summary>Independent two-character roleplay scenes. They never share normal chat memory by default.</summary>
    public List<SoulScene> Scenes { get; set; } = [];
}
...
public sealed class AppPreferences
{
    ...
    public string MobileAccessPassword { get; set; } = "admin";
    ...
    public string CognitiveBackgroundMode { get; set; } = "idle";
...
public sealed class SoulPersona
{
    ...
    [JsonIgnore]
    public string Initials => string.Concat(Name.Split(' ', StringSplitOptions.RemoveEmptyEntries).Select(part => part[0])).ToUpperInvariant();
...
public sealed class SoulCharacter
{
    ...
    public List<SoulChat> Chats { get; set; } = [];
...
public sealed class SoulChat
{
    public Guid Id { get; set; } = Guid.NewGuid();
    public string Name { get; set; } = "Основной чат";
    /// <summary>Закреплённые диалоги отображаются первыми в общем списке.</summary>
    public bool IsPinned { get; set; }
    public bool IsArchived { get; set; }
    public string SummaryText { get; set; } = "";
    public string SummaryDirectives { get; set; } = "";
    /// <summary>Initial facts about the user for this specific story branch; later dialogue and memory take priority.</summary>
    public string InitialUserProfile { get; set; } = "";
    /// <summary>Initial relationship or situation for this specific story branch; it is not a permanent fact.</summary>
    public string InitialRelationshipContext { get; set; } = "";
    public int LastSummarizedSequence { get; set; }
    public Dictionary<Guid, string> StateValuesJson { get; set; } = [];
    public List<SoulMessage> Messages { get; set; } = [];
    public SoulMemoryBundle Memory { get; set; } = new();
    public DateTimeOffset CreatedAt { get; set; } = DateTimeOffset.Now;
    public DateTimeOffset UpdatedAt { get; set; } = DateTimeOffset.Now;
    public override string ToString() => string.IsNullOrWhiteSpace(Name) ? "Чат" : Name;
}

public sealed class SoulScene
{
    public Guid Id { get; set; } = Guid.NewGuid();
    public string Name { get; set; } = "Новая сцена";
    ...
    /// <summary>Shared established relationship/context between both scene participants.</summary>
    public string RelationshipContext { get; set; } = "";
    public bool EnforceSceneContract { get; set; } = true;
    /// <summary>Encourages each turn to advance the scene and not repeat the recent dialogue.</summary>
    public bool AdvanceSceneAndAvoidRepetition { get; set; } = true;
    /// <summary>alternate or manual. Alternate advances to the other participant after each generated turn.</summary>
    public string TurnMode { get; set; } = "alternate";
    /// <summary>0 means manual, otherwise 5–30 seconds after a completed turn.</summary>
    public int DelaySeconds { get; set; } = 10;
    public string Status { get; set; } = "paused";
    public Guid? NextCharacterId { get; set; }
    /// <summary>Canonical scheduled time of the next automatic turn, shared with mobile clients.</summary>
    public DateTimeOffset? NextTurnAt { get; set; }
    public string SummaryText { get; set; } = "";
    public int LastSummarizedSequence { get; set; }
    public bool IsPinned { get; set; }
    public List<SoulSceneMessage> Messages { get; set; } = [];
    public DateTimeOffset CreatedAt { get; set; } = DateTimeOffset.Now;
    public DateTimeOffset UpdatedAt { get; set; } = DateTimeOffset.Now;
    [JsonIgnore]
    public string StatusDisplay => Status == "running" ? "Идёт" : Status == "finished" ? "Завершена" : "Пауза";
    public override string ToString() => string.IsNullOrWhiteSpace(Name) ? "Сцена" : Name;
}

public enum SoulSceneMessageKind { Character, Director }

public sealed class SoulSceneMessage
{
    public Guid Id { get; set; } = Guid.NewGuid();
    public int SequenceNumber { get; set; }
    public SoulSceneMessageKind Kind { get; set; } = SoulSceneMessageKind.Character;
