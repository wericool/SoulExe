...
public sealed class LlamaServerService : IAsyncDisposable
{
    ...
    public async IAsyncEnumerable<string> GenerateFromMessagesAsync(AppSettings settings, IReadOnlyList<LlamaMessage> messages, [System.Runtime.CompilerServices.EnumeratorCancellation] CancellationToken cancellationToken = default, string? diagnosticId = null)
    {
        ...
        while (!reader.EndOfStream)
        {
            ...
            {
                malformedEvents++;
                if (malformedEvents <= 2) AppLog.Write($"GEN {traceId} SSE_PARSE_WARNING event={dataEvents} chars={data.Length} hash={AppLog.Fingerprint(data)}");
            }
        ...
        stopwatch.Stop();
        AppLog.Write($"GEN {traceId} HTTP_STREAM_END done={doneReceived} dataEvents={dataEvents} contentChunks={contentChunks} malformed={malformedEvents} chars={receivedText.Length} hash={AppLog.Fingerprint(receivedText.ToString())} elapsedMs={stopwatch.ElapsedMilliseconds}");
    }
