...
public sealed class CharactersGatewayService
{
    ...
    public async Task<SoulCharacter> ImportChubCharacterAsync(GatewayCharacterDetails details, CancellationToken token = default)
    {
        ...
        if (!string.IsNullOrWhiteSpace(details.FirstMessage)) character.Greetings.Add(new SoulGreeting { Text = details.FirstMessage, IsPrimary = true, Position = 0 });
        for (var i = 0; i < details.AlternateGreetings.Count; i++) character.Greetings.Add(new SoulGreeting { Text = details.AlternateGreetings[i], Position = i + 1 });
        var chat = new SoulChat { Name = "Основной чат", CreatedAt = now, UpdatedAt = now };
        if (!string.IsNullOrWhiteSpace(details.FirstMessage))
        {
            var greeting = new SoulMessageVariant { Label = "Приветствие", Content = details.FirstMessage, CreatedAt = now };
            chat.Messages.Add(new SoulMessage { Role = SoulMessageRole.Assistant, AuthorName = character.Name, SequenceNumber = 1, CurrentVariantId = greeting.Id, Variants = [greeting], CreatedAt = now });
        }
    ...
    public async Task<SoulCharacter> ImportTextScenarioAsync(GatewayAssetItem item, CancellationToken token = default)
    {
        ...
        var opening = GetString(data, "opening_narration", GetString(data, "first_message"));
        ...
        if (!string.IsNullOrWhiteSpace(opening)) character.Greetings.Add(new SoulGreeting { Text = opening, IsPrimary = true, Position = 0 });
        var chat = new SoulChat { Name = title, CreatedAt = now, UpdatedAt = now };
        if (!string.IsNullOrWhiteSpace(opening))
        {
            var greeting = new SoulMessageVariant { Label = "Начало сценария", Content = opening, CreatedAt = now };
            chat.Messages.Add(new SoulMessage { Role = SoulMessageRole.Assistant, AuthorName = character.Name, SequenceNumber = 1, CurrentVariantId = greeting.Id, Variants = [greeting], CreatedAt = now });
        }
        character.Chats.Add(chat);
        character.CurrentChatId = chat.Id;
