...
public sealed class CharacterCardImportService
{
    ...
    private SoulCharacter CreateCharacter(JsonElement data, string sourcePath)
    {
        ...
        var first = Get(data, "first_mes", "first_message");
        var alternatives = data.TryGetProperty("alternate_greetings", out var greetings) && greetings.ValueKind == JsonValueKind.Array
            ? greetings.EnumerateArray().Where(x => x.ValueKind == JsonValueKind.String).Select(x => x.GetString() ?? "").Where(x => x.Length > 0).ToList()
            : [];
        ...
        if (!string.IsNullOrWhiteSpace(first)) character.Greetings.Add(new SoulGreeting { Text = first, IsPrimary = true, Position = 0 });
        for (var i = 0; i < alternatives.Count; i++) character.Greetings.Add(new SoulGreeting { Text = alternatives[i], Position = i + 1 });
        var chat = new SoulChat { Name = "Основной чат", CreatedAt = now, UpdatedAt = now };
        if (!string.IsNullOrWhiteSpace(first))
        {
            var variant = new SoulMessageVariant { Label = "Приветствие", Content = first, CreatedAt = now };
            chat.Messages.Add(new SoulMessage { Role = SoulMessageRole.Assistant, AuthorName = character.Name, SequenceNumber = 1, CurrentVariantId = variant.Id, Variants = [variant], CreatedAt = now });
        }
