...
public sealed class SceneTurnScheduler : IAsyncDisposable
{
    ...
    public SceneTurnScheduler(ConversationService conversations) => _conversations = conversations;
    ...
    public async Task ScheduleAsync(Guid sceneId, Func<Guid, CancellationToken, Task> executeDueTurn, CancellationToken token = default)
    {
        ...
        var conversation = await _conversations.GetGroupAsync(sceneId, token).ConfigureAwait(false);
        if (conversation is null || !CanRunAutomatically(conversation)) return;
    ...
    private async Task RunAsync(Guid sceneId, Func<Guid, CancellationToken, Task> executeDueTurn, CancellationTokenSource source)
    {
        try
        {
            ...
            {
                var conversation = await _conversations.GetGroupAsync(sceneId, source.Token).ConfigureAwait(false);
                if (conversation is null || !CanRunAutomatically(conversation)) return;

                var dueAt = conversation.TurnState?.NextTurnAt;
                ...
                var current = await _conversations.GetGroupAsync(sceneId, source.Token).ConfigureAwait(false);
                if (current is null || !CanRunAutomatically(current) || current.TurnState?.NextTurnAt != dueAt) continue;
    ...
    private static bool CanRunAutomatically(Models.ConversationSnapshot conversation) =>
        conversation.TurnState is { } turn && ConversationTurnPolicy.CanScheduleAutomaticTurn(turn.Status, turn.Mode, turn.DelaySeconds);
}
