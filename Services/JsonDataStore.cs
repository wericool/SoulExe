...
public sealed class JsonDataStore
{
    ...
    /// <summary>Preserves the failed file and creates an empty v10 store only after explicit recovery confirmation.</summary>
    public async Task CreateNewStoreAfterRecoveryAsync(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            if (File.Exists(_dataFile))
            {
                ...
                var preserved = Path.Combine(_backupDirectory, $"recovery_preserved_{DateTimeOffset.Now:yyyyMMdd_HHmmss_fff}.json");
                File.Copy(_dataFile, preserved, overwrite: false);
            }
            _root = CreateInitialRoot();
            await PersistUnsafeAsync("recovery_new_store", cancellationToken, createBackup: false);
        }
    ...
    public async Task InitializeAsync(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            if (_root.SchemaVersion < CurrentSchemaVersion)
                throw new InvalidOperationException($"Формат данных schema v{_root.SchemaVersion} больше не поддерживается. Для SoulExe требуется schema v{CurrentSchemaVersion}.");
    ...
    public async Task<T> MutateConversationsAsync<T>(Func<List<ConversationSnapshot>, T> mutation, string reason, CancellationToken cancellationToken = default)
    {
        await _gate.WaitAsync(cancellationToken);
        try
        {
            ...
            try
            {
                var result = mutation(root.Conversations);
                await PersistUnsafeAsync(reason, cancellationToken, createBackup: true);
                return result;
            }
            catch
    ...
    public Task MutateConversationsAsync(Action<List<ConversationSnapshot>> mutation, string reason, CancellationToken cancellationToken = default) =>
        MutateConversationsAsync(conversations => { mutation(conversations); return true; }, reason, cancellationToken);
    ...
    {
        if (!File.Exists(_dataFile)) return CreateInitialRoot();

        string json;
        try { json = await File.ReadAllTextAsync(_dataFile, cancellationToken); }
        catch (Exception exception) when (exception is IOException or UnauthorizedAccessException)
        { throw new InvalidDataException("Не удалось прочитать файл пользовательских данных. Исходный файл не изменён.", exception); }

        try
        {
            using var document = JsonDocument.Parse(json);
            if (!document.RootElement.TryGetProperty("SchemaVersion", out var version))
                throw new InvalidDataException("В файле данных отсутствует SchemaVersion.");
            if (version.GetInt32() == 8) return await MigrateV8UnsafeAsync(json, cancellationToken);
            if (version.GetInt32() != CurrentSchemaVersion)
                throw new InvalidDataException($"Формат данных schema v{version.GetInt32()} не поддерживается. Исходный файл не изменён.");
            return JsonSerializer.Deserialize<SoulDataRoot>(json, _json) ?? throw new InvalidDataException("Файл данных пуст.");
        }
        catch (JsonException exception)
        { throw new InvalidDataException("Файл данных повреждён или имеет неверный JSON. Исходный файл не изменён.", exception); }
    }

    private async Task<SoulDataRoot> MigrateV8UnsafeAsync(string legacyJson, CancellationToken cancellationToken)
    {
        var permanentBackup = Path.Combine(_backupDirectory, $"migration_v8_{DateTimeOffset.Now:yyyyMMdd_HHmmss_fff}.json");
        await File.WriteAllTextAsync(permanentBackup, legacyJson, cancellationToken);
        if (!File.Exists(permanentBackup) || !string.Equals(await File.ReadAllTextAsync(permanentBackup, cancellationToken), legacyJson, StringComparison.Ordinal))
            throw new IOException("Не удалось проверить постоянную резервную копию schema v8.");

        var migrated = SchemaV8Migration.ParseAndMigrate(legacyJson);
        var temporary = _dataFile + ".migration.tmp";
        try
        {
            await File.WriteAllTextAsync(temporary, JsonSerializer.Serialize(migrated, _json), cancellationToken);
            var roundTrip = JsonSerializer.Deserialize<SoulDataRoot>(await File.ReadAllTextAsync(temporary, cancellationToken), _json)
                ?? throw new InvalidDataException("Временный файл миграции пуст.");
            SchemaV8Migration.Validate(roundTrip, migrated.Conversations.Count, migrated.Conversations.Sum(x => x.Messages.Count));
            File.Move(temporary, _dataFile, overwrite: true);
            AppLog.Write($"STORE MIGRATION v8-to-v10 backup={permanentBackup} conversations={migrated.Conversations.Count}");
            return migrated;
        }
    ...
    private static SoulDataRoot CreateInitialRoot()
    {
        ...
        var conversation = ConversationFactory.Personal(character, "Основной разговор", character.CreatedAt);
        character.CurrentChatId = conversation.Id;
        return new SoulDataRoot { SchemaVersion = CurrentSchemaVersion, Characters = [character], Conversations = [conversation], PromptPresets = CreateBundledPromptPresets() };
    }
    ...
    private async Task PersistUnsafeAsync(string reason, CancellationToken cancellationToken, bool createBackup)
    {
        ...
        File.Move(temporary, _dataFile, overwrite: true);
        CleanupBackups();
        var bytes = File.Exists(_dataFile) ? new FileInfo(_dataFile).Length : 0;
        AppLog.Write($"STORE SAVE reason={reason} schema={GetRoot().SchemaVersion} conversations={GetRoot().Conversations.Count} bytes={bytes} elapsedMs={stopwatch.ElapsedMilliseconds} backup={createBackup}");
    }
    ...
    private async Task BackupUnsafeAsync(string reason, CancellationToken cancellationToken)
    {
        ...
        var baseName = $"soulexe_{DateTimeOffset.Now:yyyyMMdd_HHmmss_fff}_{safeReason}";
        var target = Path.Combine(_backupDirectory, baseName + ".json");
        var duplicate = 1;
        while (File.Exists(target))
            target = Path.Combine(_backupDirectory, $"{baseName}_{duplicate++}.json");
    ...
    private void CleanupBackups()
    {
        try
        {
            foreach (var file in new DirectoryInfo(_backupDirectory)
                         .EnumerateFiles("*.json")
                         .Where(file => file.Name.StartsWith("soulexe_", StringComparison.OrdinalIgnoreCase))
    ...
    private SoulDataRoot GetRoot() => _root ?? throw new InvalidOperationException("Хранилище SoulExe не инициализировано.");

    private SoulDataRoot DeepClone(SoulDataRoot source)
    {
        var serialized = JsonSerializer.Serialize(source, _json);
        return JsonSerializer.Deserialize<SoulDataRoot>(serialized, _json) ?? throw new InvalidOperationException("Не удалось скопировать данные SoulExe.");
    }
}
