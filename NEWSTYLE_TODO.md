...
- [x] **Фаза A+ — Каркас сцены и упрощение навигации** (ожидает подтверждения в EXE).
  StageShellView подключён в MainWindow. Ambient-фон (Blur+скрим), топбар:
  лого-меню, капсула присутствия, справа ШЕСТЕРЁНКА (Настройки) и чип модели
  с РАБОЧИМ flyout-меню (статус, старт/стоп, пауза загрузки, переходы в
  Модели/Настройки). Backstage v1: 4 карточки (Продолжить / Библиотека /
  Хаб / Настройки).
  НАВИГАЦИОННОЕ РЕШЕНИЕ: маршруты Models и Setup нормализуются в Options
  с соответствующей вкладкой настроек (AppNavigation.NormalizePage +
  OptionsTabForRoute); SettingsView получил 5 вкладок: Runtime / Модели /
  Быстрый старт / Оформление / Мобильный (внутри вкладок хостятся
  ModelsView и SetupView целиком). Список диалогов и сведения разговора —
  стеклянные рейки по краям сцены; композер — стеклянная капсула;
  ChatBackgroundColor по умолчанию пустой = прозрачный ambient
  (выбор цвета в Оформлении возвращает плоский фон).
- [x] **Фаза B — Сцена чата** (ожидает подтверждения в EXE): прозрачный фон тредов,
  ambient за лентой, стеклянные рейки списка/сведений, композер-капсула,
  прозрачные заголовки тредов (identity несёт пилюля присутствия), печать в пилюле.
- [x] **Фаза C — Backstage v2** (ожидает подтверждения в EXE): полноценный хаб —
  секция «Продолжить историю» (карточки ConversationItems: аватары личный/сцена,
  название, превью, тип; клик открывает разговор через SelectedConversationItem),
  галерея «Персонажи» (HomeCards; add-card уводит в Библиотеку, персонаж —
  в редактор), карточки Мир / Каталог / Модели / Настройки с подсказками.
  Фокус при открытии — MoveFocus(First) по оверлею; Esc закрывает.
- [x] **Фаза D — Листы** (ожидает подтверждения в EXE): анимации slide+fade
  (200ms ease-out вход / 150ms выход) для шторок списка и сведений в
  ChatWorkspaceView; закрытие асинхронное с защитой от гонок (повторное
  открытие заменяет анимации, callback не схлопывает новый лист; на Unload —
  жёсткий collapse). Скрим затухает вместе с листом. Поисковые панели тредов
  (личный/групповой) выезжают сверху (fade + slide -14px, XAML EnterActions).
  Shell: fade-переходы PageOverlay↔StageHost (160ms) и Backstage при открытии.
  Escape/focus-политика без изменений (работала ранее).
- [x] **Фаза E — Библиотека мира** (ожидает подтверждения в EXE): маршруты вне сцены
  рендерятся как ЛИСТ: скрим с плавным затуханием + стеклянная рамка (radius 20,
  GlassBorder, CardShadowEffect), вход slide+fade 200ms ease-out (снизу +26px),
  выход 150ms; закрытие race-safe через версию транзита (повторное открытие
  подменяет анимации, коллбек не схлопывает новый лист); на Unloaded — жёсткий
  collapse. Чип «На сцену» переехал в скрим (затухает вместе с ним). Esc закрывает
  лист -> сцена ЧЕРЕЗ всплывающее KeyDown (не Preview): вложенные диалоги
  библиотеки/чата сначала закрываются своими PreviewKeyDown-обработчиками.
  В закулисье карточка «Мир» разделена на [Лорбуки] и [Персоны] (S.Backstage.Lore/
  Personas + новые .Hint ключи RU/EN): клик выбирает вкладку библиотеки
  (SelectLibraryTabCommand) и открывает лист «Дом» (кэш единственного экземпляра
  LibraryView сохранён, дублей нет).
- [x] **Фаза F — Система** (ожидает подтверждения в EXE): Модели и Настройки уже
  рендерятся как листы через PageSheet (Фаза E). Setup overlay (Layer 5) рестайл:
  стеклянная рамка radius 20 + GlassBorder + CardShadowEffect над скримом
  #F2060810 (как Backstage), Padding 28, MaxHeight 680, фокус-скоп.
  SettingsView: убрана внешняя PanelBrush-рамка (лист даёт каркас), корень —
  прозрачный Grid Margin=20. ModelsView без изменений (уже без внешней рамки).
  Инициализационный SetupView внутри оверлея — без правок, логика сохранена.
- [x] **Фаза G — Редактор персонажа** (ожидает подтверждения в EXE):
  CharactersView уже рендерился как лист через PageSheet (Фаза E).
  Убрана внешняя PanelBrush-рамка редактора (radius 12, border) — лист
  даёт стеклянный каркас. Корень — прозрачный Grid Margin=20.
  Пустое состояние (нет персонажа) осталось Border с PanelBrush, но
  теперь оно одноуровнево со ScrollViewer внутри колонки редактора
  (Grid.Row=0, перекрывается по видимости). Футер «Сохранить» без изменений.
- [x] **Фаза H — Локализация** (ожидает подтверждения в EXE): заголовок Setup overlay
  вынесен в S.Setup.WelcomeTitle; кнопки окна (свернуть/развернуть/закрыть)
  вынесены в S.Window.Minimize/Maximize/Restore/Close; финальный grep по
  изменённым файлам (StageShellView/SettingsView/CharactersView) — новые строки
  переведены. Legacy кириллица в SettingsView (HelpHint и заголовки секций)
  осталась на доработку отдельным таском.
...
- [x] Фикс регрессии: плавающий топбар виден ТОЛЬКО на сцене (на страницах
  перекрывал тулбары и перехватывал клики); страницам отдан PageHost с
  Margin 24,16,24,20; чип «На сцену» — иконка MDL2 + текст нормальным шрифтом
  (кириллица в MDL2 = квадраты); маршрут Characters рендерит свежий
  CharactersView вместо пустого оверлея.
- [x] Фазы A–H (см. раздел 2) — «Иммерсивный театр» полностью
